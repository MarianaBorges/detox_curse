...
const MemberContext = React.createContext({} as PropsContext);

interface PropsContext {
  data: any;
  addMember: (newMember: any, callback: any) => void ;
  editMember: (id: any, editedMember: any, callback: any) => void;
  getMembers: () => void;
...
export const MemberProvider = ({ children }) => {
  ...
  const addMember = async (newMember, callback) => {
    ...
    try {
      const updatedMembers = await AsyncStorage.getItem(
        ASYNC_STORAGE_MEMBERS_KEY,
      );

      let updatedMembersNew;

      if (!updatedMembers) {
        // the first time updatedMembers should be null.
        updatedMembersNew = [tmpMember];
      } else {
        updatedMembersNew = JSON.parse(updatedMembers);
        //AKI
        updatedMembersNew = [...updatedMembers, tmpMember];
        console.log('addMembers 1', tmpMember);
        console.log('updatedMembers',updatedMembers)
      }

      console.log('addMembers 2', updatedMembersNew)

      await AsyncStorage.setItem(
        ASYNC_STORAGE_MEMBERS_KEY,
        JSON.stringify(updatedMembersNew),
      );
      dispatch({type: ADD_MEMBER_SUCCESS, updatedMembersNew});
      callback();
    } catch (e) {
