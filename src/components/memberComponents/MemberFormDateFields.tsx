...
const MemberFormDateFields = ({
  ...
  const [pickerDate, setPickerDate] = useState(date || new Date(maxDate.toString()));
  ...
  const handleCancelChange = () => {
    setShow(false);
    setDate(inputValue);
    setPickerDate(inputValue || new Date(maxDate.toString()));
      ...
      {show && (
        <View>
         {/* <DateTimePicker
            ...
            value={new Date(pickerDate) || new Date(maxDate.toString())}
            mode="date"
            is24Hour={true}
            minimumDate={
