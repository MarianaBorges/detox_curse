...
class ListPeople {
    ...
    async pressPeopleEditItem(count) {
        switch (count) {
            case 1:
               await element(by.id('peopleItem-0')).tap()
                break;
            default:
    ...
    async removePeopleItem(count) {
        switch (count) {
            case 1:
               await element(by.id('delete-peopleItem-0')).tap();
               await this.updateButton.tap();
               await expect(element(by.id('peopleItem-0'))).not.toBeVisible();
               break;
            default:
                assert.fail(`The entered ${count} is an invalid count for Members`);
                break;
